...
| 办法 | 长什么样 | 问题 |
|---|---|---|
| 哨兵值 | `rx_avg: f64 = -1.0` | 谁来记得 `-1` 是特殊的？漏判一处就当成真实速率了 |
| 另开一个布尔 | `rx_avg: f64` + `has_avg: bool` | 两个字段会不同步。只改一个的代码永远写得出来 |
| **`Option<f64>`** | `rx_avg: Option<f64>` | **拿值必须先开盒**，编译器不让你跳过 |

前两种在 C 和 Python 里天天见，它们的共同点是：
**「可能没有」这件事只写在注释里，编译器管不着。**

`Option` 把它写进了类型，于是「忘记处理」从一个运行时 bug 变成了编译错误。

> 这条规矩在这个项目里会反复出现，换着花样考你：
> 第 16 课「没采到丢包」≠「丢包为零」，
> 第 18 课「取消了没跑」≠「报告里没有这一行」。**都是同一件事。**
...
只进 `diagnostics` —— **第 16 课**专门讲这个（丢包 31% 却判 PASS）。
